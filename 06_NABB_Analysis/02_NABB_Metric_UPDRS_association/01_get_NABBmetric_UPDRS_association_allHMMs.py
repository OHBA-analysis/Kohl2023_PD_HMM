...
Script asessing association between NABB metrics and Bradykinesia/Rigidity scores.
NABB Metrics are correlated with UPDRS scores while controlling for confounds
with GLM approach.

Significance of correlations assessed with Max-Tstatistic Permutation tests
pooling across States.

This preocedure is repeated for each NABB Metric.

Script Loops across all number of states and runs for robustness check of 
NABB metric x UPDRS association.
"""
...
labels_metric = ['fractional_occupancy', 'mean_lifetimes', 
                 'mean_intervaltimes','state_rates', 'beta_pow']
    ...
    for fsample in sampling_rates:
        for emb in embeddings:
            for K in number_of_states:
                ...
                indir = proj_dir +  'Data/Burst_x_State_Metrics/ds' + \
                    str(fsample) + '/K' + str(K) + '/run' + str(irun) + '/'
                
                outdir_dat = proj_dir + 'Data/Burst_x_State_Metrics_x_UPDRS/ds' + \
                    str(fsample) + '/K' + str(K) + '/run' + str(irun) + '/'
                
                outdir_plot = proj_dir + 'Results/Burst_x_State_Metrics_x_UPDRS/ds' + \
                    str(fsample) + '/K' + str(K) + '/run' + str(irun) + '/'
                ...
                # Load Burst_x_State_Metrics
                metrics = pickle.load(open(indir + 'Burst_x_State_Metrics_GLM_Pow.mat', "rb"))['metrics']
                ...
                for iLabel in range(len(labels_metric)):
                    ...
                    # --- Load State Metrics ---
                    in_data = np.squeeze(metrics[iLabel])
